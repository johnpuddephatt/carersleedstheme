...
class Page extends Composer
{
    ...
    public function get_toc($content = '')
    {
        ...
        if ($items) {
            ...
            $toc .= '<h3 class="font-semibold mb-6">On this page</h3><ul id="toc-menu">';

            if (strpos($content, '<h2') >= 10) {
                $toc .=  '<li class="mb-4 leading-tight"><a class=" text-blue-dark" href="#overview">Overview</a></li>';
            }
            foreach ($items as $item) {
                $toc .= '<li class="mb-4 leading-tight" ><a class=" text-blue-dark" href="#' . sanitize_title_with_dashes($item[2]) . '">' . $item[2] . '</a></li>';
            }
            $toc .= '</ul>';
            $toc .= '</div>';
        }
    ...
    private function add_ids_and_jumpto_links($tag, $content)
    {
        ...
        foreach ($items as $item) {
            ...
            $replacement   .= sprintf('<div class="group  relative"><%1$s class="flex justify-between gap-2 text-blue" id="%2$s">%3$s<a href="#%2$s" class="before:absolute before:inset-0 group-hover:opacity-70 opacity-0 transition "><svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-width="1.5" class="w-6 h-6 inline-block" viewBox="0 0 24 24">
