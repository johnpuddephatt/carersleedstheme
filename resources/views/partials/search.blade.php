<div x-cloak x-data="search" class="ml-4 items-center lg:flex xl:ml-8" @keyup.escape.window="searchOpen = false"
  ...
  <button aria-label="Open search panel" title="Search"
    class="inline-flex items-center gap-1 rounded-full border-2 border-blue-light p-2 transition hover:border-blue-dark"
    x-show="searchReady" @click="searchOpen = true">
      ...
      <div x-show="searching" class="absolute inset-0 flex h-full items-center justify-center bg-white bg-opacity-50">
        ...
        <div class="flex-1 overflow-auto">
          ...
          <div x-show="results">
            ...
            <template x-for="result in results"><a {{-- x-show="['post', 'page', 'tribe_events', 'tribe_event_series'].includes(result.post_type) && ((result.post_type !== 'tribe_events') || (!result.permalink.includes('__trashed') && new Date(result._EventStartDate) > (new Date()).setHours(0,0,0,0)))" --}} :href="result.permalink"
                ...
                <div>
                  <span x-text="result.post_type"></span>
                  <span x-text="result.permalink.includes('__trashed') ?'Trashed':''"></span>
                  <span
                    x-text="new Date(result._EventStartDate) > (new Date()).setHours(0,0,0,0))) ?'Upcoming':'Past'"></span>
